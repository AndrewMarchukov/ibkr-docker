...
docker run -d \
  ...
  ghcr.io/AndrewMarchukov/ibkr:latest
```
...
services:
  ibkr:
    image: ghcr.io/AndrewMarchukov/ibkr # latest, stable, 10.21, 10.21.1p etc
    ports:
...
- Pushing the tag triggers a docker build action (`publish.yml`, based on the `docker-` prefix), which reads the version and release channel (latest/stable) from the tag, and then fetches the repo at the tag's commit, builds from the appropriate folder (latest/stable), tags and pushes the image to [ghcr.io/AndrewMarchukov/ibkr][images].

[images]: https://github.com/AndrewMarchukov/ibkr-docker/pkgs/container/ibkr
...
[tws-api-issue]: https://github.com/AndrewMarchukov/ibkr-docker/issues/16#issuecomment-1514830115
